...
extension ViewController: CameraFeedManagerDelegate {
  ...
  func didOutput(pixelBuffer: CVPixelBuffer) {
    ...
    DispatchQueue.main.async {
      ...
      guard let nums = self.result?.inferences else { return }
      self.updateGraph(numbers: nums)
    }
  }
    func updateGraph(numbers:[Float]) {
        var lineChartEntry = [ChartDataEntry]()
        for i in 0..<numbers.count {
            let value = ChartDataEntry(x: Double(i),y: Double(numbers[i] * 8.0))
        ...
        //chtChart.backgroundColor = NSUIColor.red
